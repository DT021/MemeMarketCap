...
export const TotalsChart = () => {
    ...
    if(!totals) return intoModal(<></>, "totalsChart");
    const { labels, datasets } = data;
    totals[time].forEach(item => { labels.push(item.sub); datasets[0].data.push(item.amount); });
    options.tooltips.callbacks = {
        ...
        afterLabel: function(tooltipItem, data) {
            ...
            var percent = Math.round(
                (dataset[tooltipItem['index']] / dataset.reduce((a, b) => a + b, 0)
            ) * 100);
            return '(' + percent + '%)';
        }
    }
...
let options = {
    title:{ display:true, text:'Memes Posted', fontSize:25, fontColor:'#fff' },
